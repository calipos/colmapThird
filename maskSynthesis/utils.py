...
def glob_imgs(path):
    imgs = []
    for ext in ['*.png', '*.jpg', '*.bmp', '*.JPEG', '*.JPG']:
...
def load_mask(path):
    alpha = cv2.imread(path,flags=cv2.IMREAD_GRAYSCALE)
...
def get_camera_params(uv, pose, intrinsics, device):
    ...
    pixel_points_cam = lift(x_cam, y_cam, z_cam,
                            intrinsics=intrinsics, device=device)
...
def lift(x, y, z, intrinsics, device):
    ...
    x_lift = (x - cx.unsqueeze(-1) + cy.unsqueeze(-1)*sk.unsqueeze(-1) /
              fy.unsqueeze(-1) - sk.unsqueeze(-1)*y/fy.unsqueeze(-1)) / fx.unsqueeze(-1) * z
...
def get_sphere_intersection(cam_loc, ray_directions, device, r=1.0):
    ...
    sphere_intersections[mask_intersect] = torch.sqrt(
        under_sqrt[mask_intersect]).unsqueeze(-1) * torch.Tensor([-1, 1]).to(device).float()
...
def get_depth(points, pose, device):
    ...
    if pose.shape[1] == 7:  # In case of quaternion vector representation
        cam_loc = pose[:, 4:]
        R = quat_to_rot(pose[:, :4])
        pose = torch.eye(4).unsqueeze(0).repeat(
            batch_size, 1, 1).to(device).float()
        pose[:, :3, 3] = cam_loc
        pose[:, :3, :3] = R

    points_hom = torch.cat(
        (points, torch.ones((batch_size, num_samples, 1)).to(device)), dim=2)
