    ...
    def __init__(self, input_dim=3, output_dim=128, stride=8, norm_fn='batch', dropout=0.0):
        ...
        if self.norm_fn == 'group':
            self.norm1 = nn.GroupNorm(
                num_groups=8, num_channels=self.in_planes)
            self.norm2 = nn.GroupNorm(num_groups=8, num_channels=output_dim*2)
...
class CorrBlock2:
    ...
    def sample(self, coords):
        ...
        for i in range(self.num_levels):
            ...
            centroid_lvl = coords.reshape(S*N, 1, 1, 2) / 2**i
            delta_lvl = delta.view(1, 2*r+1, 2*r+1, 2)
            coords_lvl = centroid_lvl + delta_lvl

            corrs = bilinear_sampler(corrs.reshape(S*N, 1, H, W), coords_lvl)
            corrs = corrs.view(S, N, -1)
            out_pyramid.append(corrs)

        out = torch.cat(out_pyramid, dim=-1)  # S, N, LRR*2
        return out.contiguous().float()
...
class Pips2(nn.Module):
    def __init__(self, S=8, stride=8):
        ...
        self.delta_block = DeltaBlock2(input_dim=self.latent_dim, hidden_dim=self.hidden_dim,
    ...
    def forward_fnet(self, rgbs, coords_init=None, feat_init=None, iters=3, trajs_g=None, vis_g=None, valids=None, sw=None, return_feat=False, is_train=False):
        total_loss = torch.tensor(0.0)
        ...
        #fmaps = fmaps_.reshape(S, self.latent_dim, H8, W8)
        ...
        for itr in range(iters):
            ...
            fcp = torch.zeros(
                (B, S, N, H8, W8), dtype=torch.float32, device=device)  # B,S,N,H8,W8
            for cr in range(self.corr_levels):
                # B,S,N,?,? (depending on scale)
                fcp_ = fcorr_fn.corrs_pyramid[cr]
                _, _, _, H_, W_ = fcp_.shape
                fcp_ = fcp_.reshape(B*S, N, H_, W_)
                fcp_ = F.interpolate(
            ...
            # for mixer, i want everything in the format B*N, S, C
            fcorrs_ = fcorrs.permute(0, 2, 1, 3).reshape(B*N, S, LRR)
            flows_ = (coords - coords[:, 0:1]
                      ).permute(0, 2, 1, 3).reshape(B*N, S, 2)
            times_ = torch.linspace(0, S, S, device=device).reshape(
                1, S, 1).repeat(B*N, 1, 1)  # B*N,S,1
            flows_ = torch.cat([flows_, times_], dim=2)  # B*N,S,2

            ffeats_ = ffeats.permute(0, 2, 1, 3).reshape(
                B*N, S, self.latent_dim)

            delta_all_ = self.delta_block(
                ffeats_, fcorrs_, flows_)  # B*N, S, C+2
            delta_coords_ = delta_all_[:, :, :2]
            delta_feats_ = delta_all_[:, :, 2:]
            ...
            coords = coords + \
                delta_coords_.reshape(B, N, S, 2).permute(0, 2, 1, 3)
        ...
        vis_e = self.vis_predictor(ffeats.reshape(
            B*S*N, self.latent_dim)).reshape(B, S, N)
        ...
        if trajs_g is not None:
            seq_loss = sequence_loss(
                coord_predictions, trajs_g, vis_g, valids, 0.8)
            vis_loss, _ = balanced_ce_loss(vis_e, vis_g, valids)
            ce_loss = score_map_loss(
                fcps, trajs_g/float(self.stride), vis_g, valids)
            losses = (seq_loss, vis_loss, ce_loss)
        else:
            losses = None
    ...
    def forward(self, fmaps, ffeat, iters=3, trajs_g=None, vis_g=None, valids=None, sw=None, return_feat=False, is_train=False):
        total_loss = torch.tensor(0.0)
        _, N, _ = ffeat.shape
        S, C, H8, W8 = fmaps.shape
        device = fmaps.device
 
        fcorr_fn = CorrBlock2(
            fmaps, num_levels=self.corr_levels, radius=self.corr_radius)

        # ffeat = utils.samp.bilinear_sample2d2(fmaps[0], coords[ 0, :, 0], coords[ 0, :, 1])
        ffeats = ffeat.repeat(S, 1, 1)  # B, S, N, C
        # coords_bak = coords.clone()
        ...
        fcp = torch.zeros(
            ( S, N, H8, W8), dtype=torch.float32, device=device)  # B,S,N,H8,W8
        for cr in range(self.corr_levels):
            # B,S,N,?,? (depending on scale)
            fcp_ = fcorr_fn.corrs_pyramid[cr]
            fcp_ = F.interpolate(
                fcp_, (H8, W8), mode='bilinear', align_corners=True)
            fcp = fcp + fcp_.reshape(S, N, H8, W8)
        fcps.append(fcp)
        return fcps
        ...
        # for mixer, i want everything in the format B*N, S, C
        fcorrs_ = fcorrs.permute(1, 0, 2)
        flows_ = (coords - coords[ 0:1]).permute(1, 0, 2)
        times_ = torch.linspace(0, S, S, device=device).reshape(
            1, S, 1).repeat(N, 1, 1)  # B*N,S,1
        flows_ = torch.cat([flows_, times_], dim=2)  # B*N,S,2

        ffeats_ = ffeats.permute(1, 0, 2)
        ...
        ffeats_ = ffeats_.reshape(N*S, self.latent_dim)
        delta_feats_ = delta_feats_.reshape(N*S, self.latent_dim)
        ffeats_ = self.ffeat_updater(self.norm(delta_feats_)) + ffeats_
        ffeats = ffeats_.reshape(N, S, self.latent_dim).permute(
             1, 0, 2)  # B,S,N,C

        coords = coords + delta_coords_.permute(1, 0, 2)
        coords[:, 0] = coords_bak[:, 0]  # lock coord0 for target
        return coords
