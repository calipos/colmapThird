...
class XSeg(BaseFaceParser):
    ...
    def __init__(
        ...
        *,
        ...
        Logger.info(
            f'Initializing XSeg with model={model_name}, align_size={align_size}, blur_sigma={blur_sigma}')
    ...
    def _initialize_model(self) -> None:
        ...
        try:
            ...
            if isinstance(input_shape[1], int) and isinstance(input_shape[2], int):
                ...
                Logger.info(
                    f'Dynamic input shape detected, using default: {self.input_size}')
            ...
            Logger.error(
                f"Failed to load XSeg model from '{self.model_path}'", exc_info=True)
    ...
    def parse(self, image: np.ndarray, *, landmarks: np.ndarray | None = None) -> np.ndarray:
        ...
        face_crop, inverse_matrix = face_alignment(
            image, landmarks, image_size=self.align_size)
