...
class IDRNetwork(nn.Module):
    def __init__(self, conf, device):
        ...
        self.ray_tracer = RayTracing(
            **conf.get_config('ray_tracer'), device=self.device)
    ...
    def forward(self, input):
        ...
        ray_dirs, cam_loc = rend_util.get_camera_params(
            uv, pose, intrinsics, self.device)
        ...
        #np.savetxt("ret.txt", points.numpy().reshape(-1,3),fmt='%f',delimiter=',')
        ...
        if self.training:
            ...
            eikonal_points = torch.empty(
                n_eik_points, 3).uniform_(-eik_bounding_box, eik_bounding_box).to(self.device)
