...
def plot(model, indices, model_outputs, pose, rgb_gt, path, epoch, img_res, plot_nimgs, max_depth, resolution, device):
    ...
    depth[network_object_mask] = rend_util.get_depth(
        points, pose, device).reshape(-1)[network_object_mask]
    ...
    plot_images(rgb_eval, rgb_gt, path, epoch, plot_nimgs, img_res, device)
    ...
    surface_traces = get_surface_trace(path=path,
                                       epoch=epoch,
                                       sdf=lambda x: model.implicit_network(x)[
                                           :, 0], device=device,
    ...
    for i, loc, dir in zip(indices, cam_loc, cam_dir):
        data.append(get_3D_quiver_trace(loc.unsqueeze(
            0), dir.unsqueeze(0), name='camera_{0}'.format(i)))
    ...
    for i, p, m in zip(indices, points, network_object_mask):
        ...
        data.append(get_3D_scatter_trace(
            p, name='intersection_points_{0}'.format(i), caption=caption))
    ...
    fig.update_layout(scene=scene_dict, width=1400,
...
def get_3D_scatter_trace(points, name='', size=3, caption=None):
    ...
    assert len(
        points.shape) == 2, "3d scatter plot input points are not correctely shaped "
...
def get_3D_quiver_trace(points, directions, color='#bd1540', name=''):
    ...
    assert len(
        points.shape) == 2, "3d cone plot input points are not correctely shaped "
    assert directions.shape[1] == 3, "3d cone plot input directions are not correctely shaped "
    assert len(
        directions.shape) == 2, "3d cone plot input directions are not correctely shaped "
...
def get_surface_trace(path, epoch, sdf, device, resolution=100, return_mesh=False):
    grid = get_grid_uniform(resolution, device)
...
def get_surface_high_res_mesh(sdf, device, resolution=100):
    # get low res mesh to sample point cloud
    grid = get_grid_uniform(100, device)
    z = []
    points = grid['grid_points']

    for i, pnts in enumerate(torch.split(points, 100000, dim=0)):
        z.append(sdf(pnts).detach().to(device).numpy())
    z = np.concatenate(z, axis=0)
    ...
    if (not (np.min(z) > 0 or np.max(z) < 0)):
        ...
        verts = (verts + grid_points[0]).to(device).numpy()
...
def get_grid_uniform(resolution, device):
    ...
    return {"grid_points": grid_points.to(device),
...
def plot_images(rgb_points, ground_true, path, epoch, plot_nrow, img_res, device):
