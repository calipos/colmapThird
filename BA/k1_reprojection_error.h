...
#ifndef CERES_EXAMPLES_K1_REPROJECTION_ERROR_H_
#define CERES_EXAMPLES_K1_REPROJECTION_ERROR_H_
#include <vector>
#include <numeric>
...
namespace ba {
    ...
    struct K1ReprojectionError {
        K1ReprojectionError(double observed_x, double observed_y)
        ...
        bool operator()(const T* const camera,
            ...
            const T predicted_x = distortion * xp;
            const T predicted_y = distortion * yp;
        ...
        static ceres::CostFunction* Create(const double observed_x,
            ...
            return (new ceres::AutoDiffCostFunction<K1ReprojectionError, 2, 1, 6, 3>(new K1ReprojectionError(observed_x, observed_y)));
        }
        ...
        static double figureErr(const double* objPts, const double* camerasParam, const double* imgRtParam, const double* imgPts, const int& imgPtsCnt, const int* point_index, const int* camera_index, const int* img_index)
        {
            ...
            for (int i = 0; i < imgPtsCnt; i++)
            {
                ...
                const double k1 = camerasParam[thisCameraIdx * 3];
                ...
                const double predicted_x = distortion * xp;
                const double predicted_y = distortion * yp;
    ...
    struct K1ReprojectionErrorWithQuaternions {
        K1ReprojectionErrorWithQuaternions(double observed_x, double observed_y)
            : observed_x(observed_x), observed_y(observed_y) {}
        ...
        static ceres::CostFunction* Create(const double observed_x,
            const double observed_y) {
            return (
                new ceres::AutoDiffCostFunction<K1ReprojectionErrorWithQuaternions,
                2,
                1,
                7,
                3>(
                    new K1ReprojectionErrorWithQuaternions(observed_x,
                        observed_y)));
        }
