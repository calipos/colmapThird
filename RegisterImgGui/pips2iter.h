...
#define SHOW_NCNN_BLOB(x)   
	...
	public:
			...
			const int& radius_ = 3)
		{
			...
			omega = ncnn::Mat(latent_dim / 4, 1, (size_t)4);
			double omegaEachLinear = (1. / omega.w - 1);
			for (int i = 0; i < omega.w; i++)
			{
				if (i == omega.w - 1)
				{
					((float*)omega.data)[i] = 1. / omega_temperature;
				}
				else
				{
					((float*)omega.data)[i] = 1. / std::pow(omega_temperature, static_cast<double>(i) / (omega.w - 1));
				}
			}


			positionDiffEncoderNet = std::shared_ptr<ncnn::Net>(new ncnn::Net());
			positionDiffEncoderNet->load_param_mem(getDeltaInNer(sequenceCnt).c_str());
...

			{
				...
				for (size_t i = 0; i < pyramid_level; i++)
				{
					if (i == 0)
					{
						fmapPyramidSize[i].width = imgSize.width / stride;
						fmapPyramidSize[i].height = imgSize.height / stride;
					}
					else
					{
				...
				std::string paramStr = getCorrsNet(sequenceCnt, imgSize.height, imgSize.width);
		...
		std::string getDeltaInNer(const int& sequenceLength)
		{
		...
		std::string getCorrsNet(const int& sequenceLength, const int& imgHeight, const int& imgWidth)
		{
			...
			std::string reshapeWithBatchStr = "Reshape fmaps 1 1 fmapsWithBatch fmaps 2=" + std::to_string(sequenceLength * latent_dim) + " 1=0 0=0\n";
			...
			int h0 = imgHeight / stride;
			int w0 = imgWidth / stride;
			std::string reshape0Str = "Reshape /Reshape 1 1 fmaps_splitncnn_0 /Reshape_output_0 2=" + std::to_string(sequenceLength) + " 1=" + std::to_string(latent_dim) + " 0=-1\n";
			paramStr += reshape0Str;
			paramStr += "MatMul           /MatMul                  2 1 feats_splitncnn_0 /Reshape_output_0 /MatMul_output_0\n";
			std::string div0Str = "BinaryOp  /Div  1 1 /MatMul_output_0 corrs_pyramid_reshape_0 0=3  1=1  2=" + std::to_string(sqrt(double(latent_dim))) + "\n";
			paramStr += div0Str;
			int h1 = h0 / 2;
			int w1 = w0 / 2;
			std::string reshape1Str = "Reshape /Reshape_1 1 1 /AveragePool_output_0_splitncnn_0 /Reshape_1_output_0 2=" + std::to_string(sequenceLength) + " 1=" + std::to_string(latent_dim) + " 0=-1\n";
			paramStr += reshape1Str;
			paramStr += "MatMul           /MatMul_1                2 1 feats_splitncnn_1 /Reshape_1_output_0 /MatMul_1_output_0\n";
			std::string div1Str = "BinaryOp  /Div_1  1 1 /MatMul_1_output_0 corrs_pyramid_reshape_1 0=3  1=1  2=" + std::to_string(sqrt(double(latent_dim))) + "\n";
			paramStr += div1Str;
			int h2 = h1 / 2;
			int w2 = w1 / 2;
			std::string reshape2Str = "Reshape /Reshape_2 1 1 /AveragePool_1_output_0_splitncnn_0 /Reshape_2_output_0 2=" + std::to_string(sequenceLength) + " 1=" + std::to_string(latent_dim) + " 0=-1\n";
			paramStr += reshape2Str;
			paramStr += "MatMul           /MatMul_2                2 1 feats_splitncnn_2 /Reshape_2_output_0 /MatMul_2_output_0\n";
			std::string div2Str = "BinaryOp  /Div_2  1 1 /MatMul_2_output_0 corrs_pyramid_reshape_2 0=3  1=1  2=" + std::to_string(sqrt(double(latent_dim))) + "\n";
			paramStr += div2Str;
			int h3 = h2 / 2;
			int w3 = w2 / 2;
			std::string reshape3Str = "Reshape /Reshape_3 1 1 /AveragePool_2_output_0 /Reshape_3_output_0 2=" + std::to_string(sequenceLength) + " 1=" + std::to_string(latent_dim) + " 0=-1\n";
			paramStr += reshape3Str;
			paramStr += "MatMul           /MatMul_3                2 1 feats_splitncnn_3 /Reshape_3_output_0 /MatMul_3_output_0\n";
			std::string div3Str = "BinaryOp  /Div_3  1 1 /MatMul_3_output_0 corrs_pyramid_reshape_3  0=3  1=1  2=" + std::to_string(sqrt(double(latent_dim))) + "\n";
			paramStr += div3Str;
		...
		bool initDeltaBlockNet(const int& controlPtsCnt)
		{
			if (currentControlPtsCnt != controlPtsCnt)
			{
				currentControlPtsCnt = controlPtsCnt;
				try
				{
		...
		std::string convertDeltaNet(const std::string& paramPath, const int& controlPtsCnt)
		{
			...
			while (std::getline(fin, aline))
			{
				...
				if (segs[0].compare("InstanceNorm") == 0)
				{
					...
					std::string reshape1Str = "Reshape " + (layerName + "_reshape1") + " 1 1 " + inPutName + " " + (inPutName + "_reshape") + " 2=-1 1=1 0=" + std::to_string(sequenceCnt);
					...
					std::string reshape2Str = "Reshape " + (layerName + "_reshape2") + " 1 1 " + outPutName + " " + outPutNameOld + " 2=-1 1=" + std::to_string(controlPtsCnt) + " 0=" + std::to_string(sequenceCnt);
		...
		ncnn::Mat bilinear_sample2d(const ncnn::Mat& blob, const std::vector<std::vector<float>>& xs, const std::vector<std::vector<float>>& ys, std::shared_ptr<ncnn::Net> bilinearOpNet, const int& padding_mode=0)
		{
		...
		static const int corrsBlockCnt = 3;
		static const int latent_dim = 128;
		static const int pyramid_level = 4;
		static const int omega_temperature = 10000;
	private:
		ncnn::Mat omega;
		ncnn::Net encoderNet;
	...
	public:
		int currentControlPtsCnt{-1};
		bool extractFeat(const cv::Mat& img, ncnn::Mat& feat)
		{
			if (img.rows != imgSize.height || img.cols != imgSize.width)
			{
		...
		std::vector<std::vector<float>> expandInitCoord(std::vector<float>& xs, const int& times)
		{
		...
		ncnn::Mat repeatFeat(const ncnn::Mat& feat, const int& s)
		{
			int w = feat.w;
			int h = feat.h;
			int c = feat.c;
		...
		ncnn::Mat pyramidSample(const std::vector<ncnn::Mat>& corrs_pyramids, const std::vector<std::vector<float>>& stride_x, const std::vector<std::vector<float>>& stride_y)
		{
			...
			std::array<std::vector<cv::Point2f>, sequenceCnt>& traj)
		{
			...
			std::vector<int> initFrameIdx(sequenceCnt);
			std::iota(initFrameIdx.begin(), initFrameIdx.end(), 0);
			ncnn::Mat fmaps = concatFmapsWithBatch(fmapsVec, initFrameIdx);
			ncnn::Mat feats = repeatFeat(feat0, sequenceCnt);
			std::vector<std::vector<float>>xs = expandInitCoord(xs0, sequenceCnt);
			std::vector<std::vector<float>>ys = expandInitCoord(ys0, sequenceCnt);
			...
			for (size_t iter = 0; iter < iterCnt; iter++)
			{
				...
				if (iter >= 1)
				{
					...
					ncnn::Mat fmaps2 = concatFmapsWithBatch(fmapsVec, frameIdx2);
					ncnn::Mat fmaps4 = concatFmapsWithBatch(fmapsVec, frameIdx4);
