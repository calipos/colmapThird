...
#include "insmeshcleanup.h"
#include "isnmeshdisjointset.h"
#include "tbb/tbb.h"
...
extract_graph(const MultiResolutionHierarchy& mRes, bool extrinsic, int rosy, int posy,
    ...
    const Eigen::MatrixXf& Q = mRes.Q(), & O = mRes.O(), & N = mRes.N(), & V = mRes.V();
    const Eigen::VectorXf& COw = mRes.COw();
        ...
        auto classify_edges = [&](const tbb::blocked_range<uint32_t>& range) {
            for (uint32_t i = range.begin(); i < range.end(); ++i) {
                ...
                for (Link* link = adj[i]; link != adj[i + 1]; ++link) {
                    ...
                    std::pair<Eigen::Vector3f, Eigen::Vector3f> Q_rot = compat_orientation(
                        Q.col(i), N.col(i), Q.col(j), N.col(j));

                    Float error = 0;
                    std::pair<Eigen::Vector2i, Eigen::Vector2i> shift = compat_position(
                        V.col(i), N.col(i), Q_rot.first, O.col(i),
                        V.col(j), N.col(j), Q_rot.second, O.col(j),
                        scale, inv_scale, &error);

                    Eigen::Vector2i absDiff = (shift.first - shift.second).cwiseAbs();

                    if (absDiff.maxCoeff() > 1 || (absDiff == Eigen::Vector2i(1, 1) && posy == 4))
        ...
        do {
            ...
            do {
                ...
                for (uint32_t i_id = 0; i_id < adj_new.size(); ++i_id) {
                    ...
                    const Eigen::Vector3f p_i = O_new.col(i_id);
                    for (uint32_t j = 0; j < adj_i.size(); ++j) {
                        uint32_t j_id = adj_i[j].id;
                        const Eigen::Vector3f p_j = O_new.col(j_id);
                        ...
                        for (uint32_t k = 0; k < adj_j.size(); ++k) {
                            ...
                            if (k_id == i_id)
                                continue;
                            const Eigen::Vector3f p_k = O_new.col(k_id);
                ...
                for (auto t : candidates) {
                    ...
                    const Eigen::Vector3f p_i = O_new.col(i), p_j = O_new.col(j), p_k = O_new.col(k);
                    Float a = (p_j - p_k).norm(), b = (p_i - p_j).norm(), c = (p_i - p_k).norm();
                    Float s = 0.5f * (a + b + c);
                    Float height = 2 * std::sqrt(s * (s - a) * (s - b) * (s - c)) / a;
                    ...
                    if ((p_i - p_j).norm() < thresh || (p_i - p_k).norm() < thresh) {
                        ...
                        Eigen::Vector3f n_k = N_new.col(k), n_j = N_new.col(j);
                        //Eigen::Vector3f dp = p_k - p_j, dn = n_k - n_j;
            ...
            if (posy == 4) {
                ...
                for (size_t i = 0; i < adj_new.size(); ++i) {
                    ...
                    const Eigen::Vector3f p_i = O_new.col(i);
                    for (uint32_t j = 0; j < adj_i.size(); ++j) {
                        uint32_t j_id = adj_i[j].id;
                        const Eigen::Vector3f p_j = O_new.col(j_id);
...
void extract_faces(std::vector<std::vector<TaggedLink> >& adj, Eigen::MatrixXf& O,
    Eigen::MatrixXf& N, Eigen::MatrixXf& Nf, Eigen::MatrixXu& F, int posy,
            ...
            if (verts.size() == (size_t)posy) {
                ...
                for (uint32_t i = 0; i < verts.size(); ++i) {
                    ...
                    for (int k = 0; k < posy; ++k) {
                        Eigen::Vector3f v0 = O.col(verts[(i + k) % verts.size()].first);
                        Eigen::Vector3f v1 = O.col(verts[(i + k + 1) % verts.size()].first);
                        Eigen::Vector3f v2 = O.col(verts[(i + k + 2) % verts.size()].first);
                        Eigen::Vector3f d0 = (v0 - v1).normalized();
                        Eigen::Vector3f d1 = (v2 - v1).normalized();
                        Float angle = std::acos(d0.dot(d1)) * RAD_TO_DEG;
                ...
                Eigen::Vector3f centroid_normal = Eigen::Vector3f::Zero();
    ...
    if (posy == 4 && pure_quad) {
        ...
        Eigen::VectorXu V2E, E2E, E2E_v;
        Eigen::VectorXb nonManifold, boundary;
        build_dedge(F, O, V2E, E2E, boundary, nonManifold, true);
        ...
        for (uint32_t i = 0; i < nF_old; ++i) {
            Eigen::Vector4u face = F.col(i);
            if (face[2] == face[3])
                continue;
            Eigen::Vector3f fc = O.col(face[0]) + O.col(face[1]) + O.col(face[2]) + O.col(face[3]);
            Eigen::Vector3f fc_n = N.col(face[0]) + N.col(face[1]) + N.col(face[2]) + N.col(face[3]);
            uint32_t idx_fc = nV++;
            ...
            Eigen::Vector4u idx_ecs = Eigen::Vector4u::Constant(INVALID);
            ...
            for (uint32_t j = 0; j < 4; ++j) {
                ...
                F.col(idx_f) = Eigen::Vector4u(idx_ec0, i1, idx_ec1, idx_fc);
            }
        }
    ...
    if (smooth_iterations > 0) {
        ...
        for (int it = 0; it < smooth_iterations; ++it) {
            Eigen::MatrixXf O_prime(O.rows(), O.cols());
            Eigen::MatrixXf N_prime(O.rows(), O.cols());
            std::cout << ".";
            std::cout.flush();
                ...
                [&](const tbb::blocked_range<uint32_t>& range) {
                    ...
                    for (uint32_t i = range.begin(); i != range.end(); ++i) {
                        ...
                        if (adj_new[i].size() > 0 && !is_crease) {
                            Eigen::Vector3f centroid = Eigen::Vector3f::Zero(), avgNormal = Eigen::Vector3f::Zero();
                            ...
                            Eigen::Vector3f n = cov.jacobiSvd(Eigen::ComputeFullU).matrixU().col(2).normalized();
    ...
    std::cout.flush();
    std::vector<Eigen::MatrixXf> V_vec(2), F_vec(1);
    V_vec[0].swap(O);
    V_vec[1].swap(N);
    F_vec[0].swap(Nf);
