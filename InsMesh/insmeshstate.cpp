...
MeshStats compute_mesh_stats(const Eigen::MatrixXu& F, const Eigen::MatrixXf& V,
    bool deterministic) {
    Timer<> timer;
    MeshStats stats;
    if (F.size() != 0) {
        ...
        auto map = [&](const tbb::blocked_range<uint32_t>& range, MeshStats stats) -> MeshStats {
            for (uint32_t i = range.begin(); i != range.end(); ++i) {
                const Eigen::Vector3f& v = V.col(i);
...
void compute_dual_vertex_areas(const Eigen::MatrixXu& F, const Eigen::MatrixXf& V,
    const Eigen::VectorXu& V2E, const Eigen::VectorXu& E2E,
    const Eigen::VectorXb& nonManifold, Eigen::VectorXf& A) {
    A.resize(V.cols());
    A.setZero();
        ...
        [&](const tbb::blocked_range<uint32_t>& range) {
            for (uint32_t i = range.begin(); i != range.end(); ++i) {
                ...
                do {
                    ...
                    Eigen::Vector3f v = V.col(F(edge % 3, edge / 3));
                    Eigen::Vector3f vn = V.col(F(en % 3, en / 3));
                    Eigen::Vector3f vp = V.col(F(ep % 3, ep / 3));

                    Eigen::Vector3f face_center = (v + vp + vn) * (1.0f / 3.0f);
                    Eigen::Vector3f prev = (v + vp) * 0.5f;
                    Eigen::Vector3f next = (v + vn) * 0.5f;
