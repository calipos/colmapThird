...
extern void
generate_smooth_normals(const Eigen::MatrixXu& F, const Eigen::MatrixXf& V, Eigen::MatrixXf& N,
    bool deterministic) {
...
void generate_smooth_normals(const Eigen::MatrixXu& F, const Eigen::MatrixXf& V, const Eigen::VectorXu& V2E,
        ...
        [&](const tbb::blocked_range<uint32_t>& range) {
            for (uint32_t i = range.begin(); i != range.end(); ++i) {
                ...
                do {
                    ...
                    Eigen::Vector3f d0 = V.col(F((idx + 1) % 3, edge / 3)) - V.col(i);
                    Eigen::Vector3f d1 = V.col(F((idx + 2) % 3, edge / 3)) - V.col(i);
                    Float angle = fast_acos(d0.dot(d1) / std::sqrt(d0.squaredNorm() * d1.squaredNorm()));
                ...
                N.col(i) = norm > RCPOVERFLOW ? Eigen::Vector3f(normal / norm)
...
void generate_crease_normals(Eigen::MatrixXu& F, Eigen::MatrixXf& V, const Eigen::VectorXu& _V2E,
    ...
    const Float dpThreshold = std::cos(angleThreshold * DEG_TO_RAG);
        ...
        [&](const tbb::blocked_range<uint32_t>& range) {
            for (uint32_t i = range.begin(); i != range.end(); ++i) {
                ...
                do {
                    ...
                    if (opp == INVALID) {
                        Float norm = normal.norm();
                        N.col(vertexID) = norm > RCPOVERFLOW ? Eigen::Vector3f(normal / norm)
                            : Eigen::Vector3f::UnitX();
                        break;
                    }
...
void generate_crease_normals(const Eigen::MatrixXu& F, const Eigen::MatrixXf& V,
    const Eigen::VectorXu& _V2E, const Eigen::VectorXu& E2E,
    const Eigen::VectorXb boundary,
    const Eigen::VectorXb& nonManifold, Float angleThreshold,
        ...
        [&](const tbb::blocked_range<uint32_t>& range) {
            for (uint32_t i = range.begin(); i != range.end(); ++i) {
                ...
                do {
                    ...
                    Eigen::Vector3f d0 = V.col(F((idx + 1) % 3, face)) - V.col(i);
                    Eigen::Vector3f d1 = V.col(F((idx + 2) % 3, face)) - V.col(i);
                    Float angle = fast_acos(d0.dot(d1) / std::sqrt(d0.squaredNorm() * d1.squaredNorm()));
