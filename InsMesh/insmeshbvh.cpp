...
struct Bins {
    ...
    Bins() { memset(counts, 0, sizeof(std::uint32_t) * BIN_COUNT); }
...
struct BVHBuildTask : public tbb::task {
    ...
    std::uint32_t* start, * end, * temp;

    BVHBuildTask(BVH& bvh, std::uint32_t node_idx, std::uint32_t* start, std::uint32_t* end, std::uint32_t* temp)
        : bvh(bvh), node_idx(node_idx), start(start), end(end), temp(temp) { }

    task* execute() {
        const Eigen::MatrixXu& F = *bvh.mF;
        const Eigen::MatrixXf& V = *bvh.mV;
        bool pointcloud = F.size() == 0;
        std::uint32_t size = end - start, total_size = pointcloud ? V.cols() : F.cols();
        BVHNode& node = bvh.mNodes[node_idx];

        if (size < SERIAL_THRESHOLD) {
            tbb::blocked_range<std::uint32_t> range(start - bvh.mIndices, end - bvh.mIndices);
        ...
        Float min = node.aabb.minAABB[axis], max = node.aabb.maxAABB[axis],
            inv_bin_size = Bins::BIN_COUNT / (max - min);

        Bins bins = tbb::parallel_reduce(
            tbb::blocked_range<std::uint32_t>(0u, size, GRAIN_SIZE),
            Bins(),
            [&](const tbb::blocked_range<std::uint32_t>& range, Bins result) {
                for (std::uint32_t i = range.begin(); i != range.end(); ++i) {
                    std::uint32_t f = start[i];
        ...
        for (int i = Bins::BIN_COUNT - 2; i >= 0; --i) {
            std::uint32_t prims_left = bins.counts[i], prims_right = (end - start) - bins.counts[i];
        ...
        std::uint32_t left_count = bins.counts[best_index];
        ...
        std::atomic<std::uint32_t> offset_left(0), offset_right(bins.counts[best_index]);
        tbb::parallel_for(
            tbb::blocked_range<std::uint32_t>(0u, size, GRAIN_SIZE),
            [&](const tbb::blocked_range<std::uint32_t>& range) {
                std::uint32_t count_left = 0, count_right = 0;
                for (std::uint32_t i = range.begin(); i != range.end(); ++i) {
                    std::uint32_t f = start[i];
                    Float centroid = pointcloud ? V(axis, f)
                        : ((1.0f / 3.0f) * (V(axis, F(0, f)) +
                ...
                std::uint32_t idx_l = offset_left.fetch_add(count_left);
                std::uint32_t idx_r = offset_right.fetch_add(count_right);
                for (std::uint32_t i = range.begin(); i != range.end(); ++i) {
                    std::uint32_t f = start[i];
                    Float centroid = pointcloud ? V(axis, f)
                        : ((1.0f / 3.0f) * (V(axis, F(0, f)) +
        ...
        memcpy(start, temp, size * sizeof(std::uint32_t));
    ...
    static void execute_serially(BVH& bvh, std::uint32_t node_idx, std::uint32_t* start, std::uint32_t* end, std::uint32_t* temp) {
        ...
        for (int axis = 0; axis < 3; ++axis) {
            if (pointcloud) {
                std::sort(start, end, [&](std::uint32_t f1, std::uint32_t f2) {
            ...
            Float tri_factor = BVH::T_tri / node.aabb.surfaceArea();
            for (std::uint32_t i = size - 1; i >= 1; --i) {
                std::uint32_t f = *(start + i);
                if (pointcloud) {
                    aabb.expandBy(V.col(f));
                }
                ...
                std::uint32_t prims_left = i;
                std::uint32_t prims_right = size - i;
        ...
        std::uint32_t left_count = best_index;
        int node_idx_left = node_idx + 1;
        int node_idx_right = node_idx + 2 * left_count;
        node.inner.rightChild = node_idx_right;
        node.inner.unused = 0;
...
BVH::BVH(const Eigen::MatrixXu* F, const Eigen::MatrixXf* V, const Eigen::MatrixXf* N, const AABB& aabb)
    : mIndices(nullptr), mF(F), mV(V), mN(N), mDiskRadius(0.f) {
    if (mF->size() > 0) {
        mNodes.resize(2 * mF->cols());
        memset(mNodes.data(), 0, sizeof(BVHNode) * mNodes.size());
        mNodes[0].aabb = aabb;
        mIndices = new std::uint32_t[mF->cols()];
    }
    else if (mV->size() > 0) {
        mNodes.resize(2 * mV->cols());
        memset(mNodes.data(), 0, sizeof(BVHNode) * mNodes.size());
        mNodes[0].aabb = aabb;
        mIndices = new std::uint32_t[mV->cols()];
    }
}
...
void BVH::build() {
    ...
    std::uint32_t total_size = pointcloud ? mV->cols() : mF->cols();

    for (std::uint32_t i = 0; i < total_size; ++i)
        mIndices[i] = i;

    Timer<> timer;
    std::uint32_t* temp = new std::uint32_t[total_size];
    BVHBuildTask& task = *new(tbb::task::allocate_root())
        BVHBuildTask(*this, 0u, mIndices, mIndices + total_size, temp);
    tbb::task::spawn_root_and_wait(task);
    delete[] temp;

    std::pair<Float, std::uint32_t> stats = statistics();
    std::cout << "done. ("
    ...
    std::vector<std::uint32_t> skipped_accum(mNodes.size());
    ...
    if (pointcloud) {
        ...
        auto map = [&](const tbb::blocked_range<std::uint32_t>& range, double radius_sum) -> double {
            std::vector<std::pair<Float, std::uint32_t>> result;
            for (std::uint32_t i = range.begin(); i < range.end(); ++i) {
                Float radius = std::numeric_limits<double>::infinity();
                if (findNearest(mV->col(i), radius) != (std::uint32_t)-1)
                    radius_sum += radius;
            }
        ...
        tbb::blocked_range<std::uint32_t> range(0u, (std::uint32_t)mV->cols(), GRAIN_SIZE);
...
bool BVH::rayIntersect(Ray ray, std::uint32_t& idx, Float& t, Eigen::Vector2f* uv) const {
    if (mNodes.empty())
        return false;

    std::uint32_t node_idx = 0, stack[64];
    std::uint32_t stack_idx = 0;
    bool hit = false;
    ...
    if (mF->size() > 0) {
        while (true) {
            ...
            if (node.isInner()) {
                ...
                Float _t;
                Eigen::Vector2f _uv;
                for (std::uint32_t i = node.start(), end = node.end(); i < end; ++i) {
...
void BVH::findNearestWithRadius(const Eigen::Vector3f& p, Float radius,
    ...
    while (true) {
        ...
        if (node.isInner()) {
            ...
            std::uint32_t start = node.leaf.start, end = start + node.leaf.size;
            for (std::uint32_t i = start; i < end; ++i) {
                std::uint32_t f = mIndices[i];
                Eigen::Vector3f pointPos = Eigen::Vector3f::Zero();
                if (mF->size() > 0) {
                    for (int j = 0; j < 3; ++j)
...
std::uint32_t BVH::findNearest(const Eigen::Vector3f& p, Float& radius, bool includeSelf) const {
    std::uint32_t node_idx = 0, stack[64];
    std::uint32_t stack_idx = 0;
    Float radius2 = radius * radius;
    std::uint32_t result = (std::uint32_t)-1;
    ...
    while (true) {
        ...
        if (node.isInner()) {
            std::uint32_t left = node_idx + 1, right = node.inner.rightChild;
...
void BVH::findKNearest(const Eigen::Vector3f& p, std::uint32_t k, Float& radius,
    std::vector<std::pair<Float, std::uint32_t>>& result,
    bool includeSelf) const {
    result.clear();
    ...
    Float radius2 = radius * radius;
    bool isHeap = false;
    auto comp = [](const std::pair<Float, std::uint32_t>& v1, const std::pair<Float, std::uint32_t>& v2) {
...
void BVH::findKNearest(const Eigen::Vector3f& p, const Eigen::Vector3f& n, std::uint32_t k,
    Float& radius,
    std::vector<std::pair<Float, std::uint32_t> >& result,
    ...
    Float radius2 = radius * radius;
    bool isHeap = false;
    angleThresh = std::cos(angleThresh * 0.01745329251994329576923690768489);
    auto comp = [](const std::pair<Float, std::uint32_t>& v1, const std::pair<Float, std::uint32_t>& v2) {
        return v1.first < v2.first;
    };
    ...
    while (true) {
        ...
        if (node.isInner()) {
            ...
            for (std::uint32_t i = start; i < end; ++i) {
                ...
                Eigen::Vector3f pointNormal = Eigen::Vector3f::Zero();
                if (mF->size() > 0) {
                    for (int j = 0; j < 3; ++j)
...
bool BVH::rayIntersectTri(const Ray& ray, std::uint32_t i, Float& t, Eigen::Vector2f& uv) const {
    const Eigen::Vector3f& p0 = mV->col((*mF)(0, i)),
        & p1 = mV->col((*mF)(1, i)),
        & p2 = mV->col((*mF)(2, i));
...
bool BVH::rayIntersectDisk(const Ray& ray, std::uint32_t i, Float& t) const {
    Eigen::Vector3f v = mV->col(i), n = mN->col(i);
...
void BVH::printStatistics() const {
    ...
    std::cout << "    Index buffer       : " << memString(sizeof(std::uint32_t) * mF->size()) << std::endl;
    std::cout << "    Total              : "
        << memString(sizeof(BVHNode) * mNodes.size() + sizeof(std::uint32_t) * mF->size()) << std::endl;
}

std::pair<Float, std::uint32_t> BVH::statistics(std::uint32_t node_idx) const {
    const BVHNode& node = mNodes[node_idx];
    if (node.isLeaf()) {
        return std::make_pair(T_tri * node.leaf.size, 1u);
    }
    else {
        std::pair<Float, std::uint32_t> stats_left = statistics(node_idx + 1u);
        std::pair<Float, std::uint32_t> stats_right = statistics(node.inner.rightChild);
...
void BVH::refitBoundingBoxes(std::uint32_t node_idx) {
    BVHNode& node = mNodes[node_idx];
    if (node.isLeaf()) {
        for (std::uint32_t i = node.start(); i < node.end(); ++i) {
            std::uint32_t j = mIndices[i];
            const Eigen::Vector3f& p = mV->col(j), & n = mN->col(j);
            Eigen::Vector3f s, t;
        ...
        std::uint32_t left = node_idx + 1u, right = node.inner.rightChild;
