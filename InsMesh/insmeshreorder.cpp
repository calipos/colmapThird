...
void reorder_mesh(Eigen::MatrixXu& F, std::vector<Eigen::MatrixXf>& V_vec, std::vector<Eigen::MatrixXf>& F_vec) {
    /* Build a directed edge data structure */
    Eigen::VectorXu V2E, E2E;
    Eigen::VectorXb boundary, nonManifold;
    build_dedge(F, V_vec[0], V2E, E2E, boundary, nonManifold, true);
    ...
    Eigen::MatrixXu Fp(F.rows(), F.cols());
    std::vector<Eigen::MatrixXf> V_new(V_vec.size()), F_new(F_vec.size());
...
void replicate_vertices(Eigen::MatrixXu& F, std::vector<Eigen::MatrixXf>& V) {
    std::vector<Eigen::MatrixXf> R(V.size());
