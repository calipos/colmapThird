...
AdjacencyMatrix generate_adjacency_matrix_uniform(
    const Eigen::MatrixXu& F, const Eigen::VectorXu& V2E, const Eigen::VectorXu& E2E,
    const Eigen::VectorXb& nonManifold) {
    Eigen::VectorXu neighborhoodSize(V2E.size() + 1);
...
AdjacencyMatrix
generate_adjacency_matrix_cotan(const Eigen::MatrixXu& F, const Eigen::MatrixXf& V,
        ...
        [&](const tbb::blocked_range<uint32_t>& range) {
            for (uint32_t i = range.begin(); i != range.end(); ++i) {
                ...
                do {
                    ...
                    if (it == 0) {
                        Eigen::Vector3f p = V.col(F(next_idx, f)),
                    ...
                    uint32_t opp = E2E[edge], next = dedge_next_3(opp);
                    if (opp == INVALID || next != stop) {
                        Eigen::Vector3f p = V.col(F(prev_idx, f)),
...
AdjacencyMatrix generate_adjacency_matrix_pointcloud(
    Eigen::MatrixXf& V, Eigen::MatrixXf& N, const BVH* bvh, MeshStats& stats, uint32_t knn_points,
    bool deterministic) {
    Timer<> timer;
    std::cout << "Generating adjacency matrix .. ";
    std::cout.flush();
    ...
    Eigen::VectorXu adj_size(V.cols());
    tbb::parallel_for(
        tbb::blocked_range<uint32_t>(0u, (uint32_t)V.cols(), GRAIN_SIZE),
        [&](const tbb::blocked_range<uint32_t>& range) {
    ...
    for (uint32_t i = 0; i < V.cols(); ++i) {
        ...
        if (dsetSize < V.cols() * 0.01f) {
            adj_size[i] = INVALID;
            V.col(i) = Eigen::Vector3f::Constant(1e6);
            continue;
        }
        nLinks += adj_size[i];
    }
    ...
    Eigen::VectorXu adj_offset(V.cols());
    adj_offset.setZero();
    ...
    /* Use a heuristic to estimate some useful quantities for point clouds (this
       ...
       reasonably accurate) */
    stats.mSurfaceArea = PI * stats.mAverageEdgeLength * stats.mAverageEdgeLength * 0.5f * V.cols();
