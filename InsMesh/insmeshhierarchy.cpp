...
AdjacencyMatrix downsample_graph(const AdjacencyMatrix adj, const Eigen::MatrixXf& V,
    const Eigen::MatrixXf& N, const Eigen::VectorXf& A,
    Eigen::MatrixXf& V_p, Eigen::MatrixXf& N_p, Eigen::VectorXf& A_p,
    Eigen::MatrixXu& to_upper, Eigen::VectorXu& to_lower,
    bool deterministic) {
    struct Entry {
        uint32_t i, j;
        float order;
    ...
    tbb::parallel_for(
        ...
        [&](const tbb::blocked_range<uint32_t>& range) {
            for (uint32_t i = range.begin(); i != range.end(); ++i) {
                ...
                Eigen::Vector3f normal = N.col(e.i) * area1 + N.col(e.j) * area2;
                Float norm = normal.norm();
                N_p.col(i) = norm > RCPOVERFLOW ? Eigen::Vector3f(normal / norm)
    ...
    Eigen::VectorXu neighborhoodSize(V_p.cols() + 1);
...
void MultiResolutionHierarchy::build(bool deterministic) {
    ...
    if (deterministic)
        generate_graph_coloring_deterministic(mAdj[0], mV[0].cols(), phases);
    else
        generate_graph_coloring(mAdj[0], mV[0].cols(), phases);
    mPhases.push_back(phases);

    mTotalSize = mV[0].cols();
    mCO.push_back(Eigen::MatrixXf());
    mCOw.push_back(Eigen::VectorXf());
    mCQ.push_back(Eigen::MatrixXf());
    mCQw.push_back(Eigen::VectorXf());

    std::cout << "Building multiresolution hierarchy .." << std::endl;
    Timer<> timer;
    for (int i = 0; i < MAX_DEPTH; ++i) {
        std::vector<std::vector<uint32_t>> phases_p;
        Eigen::MatrixXf N_p, V_p;
        Eigen::VectorXf A_p;
        Eigen::MatrixXu toUpper;
        Eigen::VectorXu toLower;
        ...
        if (deterministic)
            generate_graph_coloring_deterministic(adj_p, V_p.cols(), phases_p);
        else
            generate_graph_coloring(adj_p, V_p.cols(), phases_p);
...
void init_random_tangent(const Eigen::MatrixXf& N, Eigen::MatrixXf& Q) {
...
void init_random_position(const Eigen::MatrixXf& P, const Eigen::MatrixXf& N, Eigen::MatrixXf& O, Float scale) {
...
void MultiResolutionHierarchy::load(const Serializer& serializer) {
    ...
    for (uint32_t i = 0; i < levels; ++i) {
        serializer.pushPrefix(std::to_string(i));
        Eigen::MatrixXf V, N, O, Q, CQ, CO;
        Eigen::VectorXf A, CQw, COw;
        std::vector<std::vector<uint32_t>> phases;
...
void MultiResolutionHierarchy::propagateSolution(int rosy) {
    ...
    for (int l = 0; l < levels() - 1; ++l) {
        const Eigen::MatrixXf& N = mN[l];
        const Eigen::MatrixXf& N_next = mN[l + 1];
        const Eigen::MatrixXf& Q = mQ[l];
        Eigen::MatrixXf& Q_next = mQ[l + 1];

        tbb::parallel_for(
            tbb::blocked_range<uint32_t>(0u, size(l + 1), GRAIN_SIZE),
            [&](const tbb::blocked_range<uint32_t>& range) {
                for (uint32_t i = range.begin(); i != range.end(); ++i) {
                    Eigen::Vector2u upper = toUpper(l).col(i);
                    Eigen::Vector3f q0 = Q.col(upper[0]);
                    Eigen::Vector3f n0 = N.col(upper[0]);
                    Eigen::Vector3f q;

                    if (upper[1] != INVALID) {
                        Eigen::Vector3f q1 = Q.col(upper[1]);
                        Eigen::Vector3f n1 = N.col(upper[1]);
...
void MultiResolutionHierarchy::propagateConstraints(int rosy, int posy) {
    ...
    for (int l = 0; l < levels() - 1; ++l) {
        const Eigen::MatrixXf& N = mN[l];
        const Eigen::MatrixXf& N_next = mN[l + 1];
        const Eigen::MatrixXf& V = mV[l];
        const Eigen::MatrixXf& V_next = mV[l + 1];
        const Eigen::MatrixXf& CQ = mCQ[l];
        Eigen::MatrixXf& CQ_next = mCQ[l + 1];
        const Eigen::VectorXf& CQw = mCQw[l];
        Eigen::VectorXf& CQw_next = mCQw[l + 1];
        const Eigen::MatrixXf& CO = mCO[l];
        Eigen::MatrixXf& CO_next = mCO[l + 1];
        const Eigen::VectorXf& COw = mCOw[l];
        Eigen::VectorXf& COw_next = mCOw[l + 1];
        ...
        tbb::parallel_for(
            ...
            [&](const tbb::blocked_range<uint32_t>& range) {
                for (uint32_t i = range.begin(); i != range.end(); ++i) {
                    Eigen::Vector2u upper = toUpper(l).col(i);
                    Eigen::Vector3f cq = Eigen::Vector3f::Zero(), co = Eigen::Vector3f::Zero();
                    ...
                    if (co != Eigen::Vector3f::Zero()) {
                        Eigen::Vector3f n = N_next.col(i), v = V_next.col(i);
                        co -= n.dot(cq - v) * n;
                    }
#if 0
                    cqw *= 0.5f;
