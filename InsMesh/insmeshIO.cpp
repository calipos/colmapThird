...
void load_ply(const std::string& filename, Eigen::MatrixXu& F, Eigen::MatrixXf& V,
    Eigen::MatrixXf& N, bool pointcloud) {
    ...
    struct VertexCallbackData {
        Eigen::MatrixXf& V;
        VertexCallbackData(Eigen::MatrixXf& V)
            : V(V) { }
    };

    struct FaceCallbackData {
        Eigen::MatrixXu& F;
        FaceCallbackData(Eigen::MatrixXu& F)
            : F(F) { }
    };

    struct VertexNormalCallbackData {
        Eigen::MatrixXf& N;
        VertexNormalCallbackData(Eigen::MatrixXf& _N)
            : N(_N) { }
    };
...
void write_ply(const std::string& filename, const Eigen::MatrixXu& F,
    const Eigen::MatrixXf& V, const Eigen::MatrixXf& N, const Eigen::MatrixXf& Nf, const Eigen::MatrixXf& UV,
...
void load_obj(const std::string& filename, Eigen::MatrixXu& F, Eigen::MatrixXf& V) {
    ...
    std::unordered_map<obj_vertex, uint32_t, ObjVertexHash> vertexMap;
...
void load_pointcloud(const std::string& filename, Eigen::MatrixXf& V, Eigen::MatrixXf& N) {
    std::ifstream is(filename);
    if (is.fail())
    ...
    for (uint32_t i = 0; i < nFiles; ++i) {
        ...
        Eigen::MatrixXu F_sub;
        Eigen::MatrixXf V_sub, N_sub;
...
void write_obj(const std::string& filename, const Eigen::MatrixXu& F,
    const Eigen::MatrixXf& V, const Eigen::MatrixXf& N, const Eigen::MatrixXf& Nf,
    const Eigen::MatrixXf& UV, const Eigen::MatrixXf& C) {
    Timer<> timer;
    std::cout << "Writing \"" << filename << "\" (V=" << V.cols()
        << ", F=" << F.cols() << ") .. ";
    std::cout.flush();
...
void load_mesh_or_pointcloud(const std::string& filename, Eigen::MatrixXu& F, Eigen::MatrixXf& V, Eigen::MatrixXf& N) {
    std::string extension;
    if (filename.size() > 4)
        extension = str_tolower(filename.substr(filename.size() - 4));

    if (extension == ".ply")
        load_ply(filename, F, V, N, false);
    else if (extension == ".obj")
        load_obj(filename, F, V);
    else if (extension == ".aln")
        load_pointcloud(filename, V, N);
    else
        throw std::runtime_error("load_mesh_or_pointcloud: Unknown file extension \"" + extension + "\" (.ply/.obj/.aln are supported)");
}
void write_mesh(const std::string& filename, const Eigen::MatrixXu& F,
    const Eigen::MatrixXf& V, const Eigen::MatrixXf& N, const Eigen::MatrixXf& Nf,
    const Eigen::MatrixXf& UV, const Eigen::MatrixXf& C) {
    std::string extension;
    if (filename.size() > 4)
